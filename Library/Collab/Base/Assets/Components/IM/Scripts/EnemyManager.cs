...
public class EnemyManager : MonoBehaviour {
    ...
    Vector3 balancePower(Vector3 enemyPosition) {
        ...
        if (grid.getPositionPower (ranPosition) <= 2.0f) {
			return ranPosition;
		} else {
            Debug.Log("hih");
		...
		while (true) {
			...
			if (enemyCount != maxEnemies) {
				...
				if (xy [0] != -1 && xy [1] != -1) {
                    ...
                    int val2 = Mathf.RoundToInt(Random.Range(1.0f, 3.0f));
                    Vector3 newPosition = spawnPts.GetChild (val).position;
                    Transform newEnemy;
                    if (val2 == 1)
                    {
                        newEnemy = Instantiate(enemyObject1, newPosition + new Vector3(0,2,0), Quaternion.Euler(new Vector3(0, 0, 0)));

                    } else if (val2 == 2)
                    {
                        newEnemy = Instantiate(enemyObject2, newPosition + new Vector3(0, 2, 0), Quaternion.Euler(new Vector3(0, 0, 0)));
                    } else
                    {
                        newEnemy = Instantiate(enemyObject3, newPosition + new Vector3(0, 2, 0), Quaternion.Euler(new Vector3(0, 0, 0)));
                    }
